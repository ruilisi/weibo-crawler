...
//DB gorm pointer for other packages
var DB *gorm.DB
...
func Create() {
	...
	if uri, err := url.Parse(dbURL); err != nil {
		...
		if conf.GetEnv("GIN_ENV") != "production" {
			baseDb.Exec(fmt.Sprintf("CREATE DATABASE %s;", "go_crawler_test"))
		}
	}
}
...
func DropCateGory() {
	...
	DB.Table("tags").Where("category_id != 0").Update("category_id", nil)
}
...
func Drop() {
	...
	if uri, err := url.Parse(dbURL); err != nil {
		...
		if conf.GetEnv("GIN_ENV") != "production" {
			baseDb.Exec(fmt.Sprintf("DROP DATABASE %s;", "go_crawler_test"))
		}
	}
}
